...
# Test 1: Only fc1 in network.4
n4_fc1 = [n.name for n in m.graph.node if 'network.4' in n.name and 'fc1' in n.name]
n4_fc2 = [n.name for n in m.graph.node if 'network.4' in n.name and 'fc2' in n.name]
matmuls = [n.name for n in m.graph.node if n.op_type in ('MatMul', 'Gemm')]

for name, target_nodes in [
    ("matmuls_only", matmuls),
    ("matmuls_plus_n4_fc1", matmuls + n4_fc1),
    ("matmuls_plus_n4_fc2", matmuls + n4_fc2),
    ("matmuls_plus_n4_fc1_fc2", matmuls + n4_fc1 + n4_fc2),
]:
    nodes_to_exclude = [n.name for n in m.graph.node if n.name not in target_nodes]
    p_out = f"scratch/test_{name}.onnx"
    calib.rewind()
    quantize_static(
        model_input=p_fp32,
        model_output=p_out,
    ...
    print(f"{name:<25}: size={sz:5.1f} MB (<45MB: {sz<45}) | Cosine sim={cos:.5f} (>0.95: {cos>0.95})")
