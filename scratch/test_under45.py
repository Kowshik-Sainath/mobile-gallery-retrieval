...
for n in m.graph.node:
    ...
    # 1x1 convs in network.5
    if 'network.5' in n.name and ('fc1' in n.name or 'fc2' in n.name):
        target_nodes.append(n.name)
    # 1x1 convs in network.3
    if 'network.3' in n.name and ('fc1' in n.name or 'fc2' in n.name):
        target_nodes.append(n.name)
    # head
    if 'head' in n.name:
        target_nodes.append(n.name)
    # conv_exp.se (expand & reduce)
    if 'conv_exp.se' in n.name:
        target_nodes.append(n.name)

nodes_to_exclude = [n.name for n in m.graph.node if n.name not in target_nodes]

p_out = "scratch/photo_under45.onnx"
calib.rewind()
quantize_static(
    model_input=p_fp32,
    model_output=p_out,
...
print(f"RESULT: size={sz:.2f} MB (Strictly < 45 MB: {sz < 45.0}) | Cosine sim={cos:.5f} (Gate > 0.95: {cos > 0.95})")
